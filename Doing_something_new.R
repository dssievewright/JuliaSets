...
if (!file.exists("Julia.R")) {
    download.file(
        "https://raw.githubusercontent.com/dssievewright/JuliaSets/master/Julia.R",
        "Julia.R"
    )
}
source("Julia.R")
...
xseq <- function(resolution = 0.0003125,
                 xview = c(-1.5, 1.5)) {
    seq(xview[1], xview[2], by = resolution)
}
xvals <- xseq(resolution = 0.001)
yseq <- function(resolution = 0.0004,
                 yview = c(-1.1, 1.1)) {
...
m2 <-
    matrix(1:(length(xvals) * length(yvals)),
           nrow = length(xvals),
           ncol = length(yvals))
...
bmp("neg4Pos6ExtLargeNewTry.bmp",
    width = 3840,
    height = 2160)
par(bty = "n")
image(Re(m2),
